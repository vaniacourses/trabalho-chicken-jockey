...
public class RecebimentoServiceSeleniumTest {
    ...
    @Test
	public void testRealizarRecebimentoSemValor() {
		WebElement dropdown = wait.until(
            ExpectedConditions.elementToBeClickable(By.cssSelector(".btn.dropdown-toggle.selectpicker.btn-default")));
        dropdown.click();
		...
		wait.until(ExpectedConditions.alertIsPresent());
		org.openqa.selenium.Alert alert = driver.switchTo().alert();
		assertTrue(alert.getText().contains("Valor de recebimento inválido") || alert.getText().toLowerCase().contains("valor inválido"),
				"Mensagem de erro deve aparecer após tentar realizar recebimento sem valor.");
		alert.accept();
	}
